...
(c/make-type BinomialHeap [head tail] {:empty-name empty-heap :make-lazy? true}
...
(extend-type LazyBinomialHeap
  p/Insert
  (insert [h x] ($ (insert-tree (->Node 0 x empty-heap) @h)))
  p/Heap
  (merge [h1 h2] ($ (p/merge @h1 @h2)))
  (find-min [h] (:elem (first (remove-min-tree @h))))
  (delete-min [h] (let [[ts1 ts2] (remove-min-tree @h)]
...
(p/->structure (p/delete-min (->heap [1 2 -10 22 12])))

(p/find-min (p/delete-min (p/delete-min (->heap [1 2 -10 22 12]))))

(p/merge
 (->heap [1 2 -10 22 12])
 (p/insert (p/insert empty-heap 111) -222))
